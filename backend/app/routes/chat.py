...
async def chat_session_websocket_endpoint(websocket: WebSocket, chat_index: int):
    ...
    if not session:
        await websocket.send_text(
            json.dumps(
                {"type": "error", "content": "Chat session not found", "done": True}
            )
        )
        return
...
async def create_chat_session(request: ChatSessionRequest):
    ...
    try:
        ...
        chat_index = (
            get_next_chat_index() - 1
        )  # Since get_next_chat_index returns the next available index
        return {
            "status": "success",
            "session": {
                "id": session.id,
...
async def update_chat_session_title_endpoint(
    chat_index: int, request: ChatSessionUpdateRequest
):
...
async def add_message_to_chat_session_endpoint(
    chat_index: int, request: ChatMessageRequest
):
...
async def get_chat_session_messages(chat_index: int, limit: int, offset: int):
