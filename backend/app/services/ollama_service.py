...
class OllamaService:
    ...
    async def query_ollama(self, prompt: str, timeout: float, model: Optional[str] = None) -> str:
        ...
        try:
            ...
            raise Exception(f"Could not connect to Ollama at {self.generate_url}. Make sure Ollama is running: {str(e)}")
            ...
            if e.response.status_code == 404:
                ...
                raise Exception(f"Model '{model}' not found. Available models: {available_models}")
    ...
    async def query_ollama_stream(self, prompt: str, timeout: float, model: Optional[str] = None) -> AsyncGenerator[str, None]:
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return {"status": f"error_{response.status_code}", "error": response.text}
        except Exception as e:
